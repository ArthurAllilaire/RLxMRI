...
reports ${\approx}0.1\%$ agreement with established simulators such as
JEMRIS~\cite{stoecker_jemris_2010}, but those benchmarks were run on short
sequences; they should be re-checked at the long cumulative times stressed here,
and a regression test for long-duration sequences with hard RF pulses should be
added so the failure is caught before it silently degrades output.
