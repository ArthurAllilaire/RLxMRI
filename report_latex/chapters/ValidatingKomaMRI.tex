...
This chapter delivers A2, the validated Bloch-simulation pipeline, and
together with the digital twin of Chapter~\ref{ch:mri-system-phantom} (A1) it
answers C1: simulator validation for qMRI. The key contribution is the
validation itself rather than the upstream reports alone. The two failures
were reduced to minimal reproducers, traced to specific floating-point timing
mechanisms, fixed upstream, and then verified by recovering the phantom's
known $T_1$ values in the full qMRI pipeline. The hardest part was
diagnostic, not coding: the symptom was a biased $T_1$ fit that looked exactly
like an MRI-physics error (imperfect spoiling, residual transverse
magnetisation), and the chapter shows how that false lead was ruled out
before the true cause --- a relative-precision collapse of a fixed absolute
$\epsilon$ time marker at large cumulative time --- was isolated. To our
knowledge this floating-point failure mode has not been reported for an MRI
simulator before.

This validation is deliberately different from published simulator
benchmarking. KomaMRI's own publication compared it against the established
simulators JEMRIS and MRiLab, reporting mean absolute signal differences
below 0.1\% versus
JEMRIS~\cite{komaMRI:23,stoecker_jemris_2010,liu_mrilab_2017}, so
cross-simulator agreement on short reference sequences is already
established. What those benchmarks do not test is the level this project
depends on: whether a long sequence assembled from many adaptive blocks
still preserves quantitative $T_1$ recovery after simulation,
reconstruction, ROI extraction, and fitting
(Table~\ref{tab:komamri-validation-levels}). A timing error of one RF sample
biases the fitted $T_1$ while the image remains visually plausible, which is
exactly the failure class the experiments below exposed.
...
\midrule
Cross-simulator benchmark & KomaMRI vs JEMRIS/MRiLab~\cite{komaMRI:23} &
Bloch-solution agreement, speed, usability on reference sequences &
Long-cumulative-time behaviour; project-specific pipelines \\
Qualitative image check & Reconstructed phantom looks plausible &
Geometry, rough contrast, reconstruction conventions &
Systematic signal errors that bias $T_1$/$T_2$ fits \\
This chapter & Noise-free IR-SE $T_1$ recovery on the digital T1 plate &
End-to-end quantitative correctness at long cumulative sequence times &
Hardware imperfections; other sequence families \\
\bottomrule
\end{tabularx}
\captionof{table}{Levels of simulator validation. Published benchmarks cover
the first row; qMRI requires the third.}
\label{tab:komamri-validation-levels}
\endgroup
\end{center}
...
was not hiding a sign or phase convention error. The in-house FFT
reconstruction was further cross-checked against MRIReco.jl, a general Julia
MRI reconstruction framework~\cite{knopp_mrireco_2021}: for the fully sampled
Cartesian data used here the two agreed, ruling out the reconstruction
conventions as the cause. (The in-house path is kept in the RL loop because
MRIReco's encoding-operator construction added roughly 60\% overhead per
reconstruction, paid at every environment step.) These tests changed details
of the images, but not the failure mode.
...
\label{sec:komamri-limitations}

The timing failure has implications beyond KomaMRI. The same mechanism ---
Float64 spacing overrunning a fixed absolute $\epsilon$ at long cumulative time
--- could silently degrade other Bloch simulators, most of which are checked
only against plausible-looking images. KomaMRI's own published validation
reports ${\approx}0.1\%$ mean-absolute-difference agreement with
JEMRIS~\cite{komaMRI:23,stoecker_jemris_2010}, yet that agreement did not
surface the failure seen here. The reason is visible in the benchmark design:
its five accuracy experiments are all \emph{single-shot} gradient-echo
acquisitions (EPI at $\mathrm{TE}=100$\,ms, spiral at $\mathrm{TE}=0.1$\,ms),
each only of order $100$\,ms in total duration. They use hard RF pulses ---
matching the regime stressed here on that axis --- but never reach the
multi-second cumulative times that trigger the collapse, and the metric itself
is a global mean absolute difference over normalised k-space signal, which can
average out a per-sample timing drift that nonetheless biases a fitted $T_1$.

A first question for future work is therefore whether extending those
benchmarks to long-duration sequences would expose the same error, or whether
it is present but masked in the existing single-shot comparisons. Establishing
this means re-running the long-duration, hard-RF case against other simulators
(JEMRIS, MRiLab) to see whether the defect is specific to KomaMRI or a shared
blind spot. Where it is replicated, the upstream fixes and long-duration
regression tests contributed to KomaMRI here should be contributed to those
simulators too, pinning quantitative recovery accuracy at known-good operating
points so the failure is caught before it silently degrades output.

Broader validation is needed before the simulator is trusted for other qMRI
pipelines. The no-noise recovery check should be repeated with shaped (sinc) RF
pulses and slice selection, not only hard pulses --- the regime where the timing
error becomes a smaller perturbation (Section~\ref{subsec:komamri-why-qmri}) but
other approximations enter. This is a prerequisite for the action-space and
slice-selective extensions proposed for the RL agent
(Chapter~\ref{ch:adaptive-rl}): those would have to be validated here first.

\section{Summary}
\label{sec:komamri-summary}

This chapter delivered A2 and, with the twin, completes the answer to C1. The
qMRI validation that A1 makes possible --- demanding correct $T_1$ recovery
from a known phantom rather than a plausible-looking image --- exposed two
floating-point time-discretisation bugs in KomaMRI that only surface beyond
${\sim}128$ s of cumulative sequence time, precisely the regime that
RL-driven long acquisitions reach. Both were isolated to minimal reproducers,
explained by the relative precision of Float64 overrunning a fixed absolute
$\epsilon$ marker, fixed upstream, and verified end to end: the no-noise
14-sphere fit improved from 39.4\% to 0.48\% mean MAPE. The lasting lessons
are that quantitative validation catches systematic errors that qualitative
image inspection hides, and that a plausible physical explanation (spoiling)
must still be ruled out rather than assumed. The corrected simulator is the
trusted foundation for the adaptive experiments of
