...
maintained MRI simulation framework~\cite{komaMRI:23}, my validation experiments
found two floating-point time-discretisation failures.
Both were triggered by long cumulative sequence time. The bugs were reduced
...
although in reality a 180 degree refocus pulse can vary by a couple of degrees
...
test simulated an inversion-recovery sweep across the 14 T1 spheres and fitted
...
$T_\mathrm{RF}=1$ ms, one missing sample is 5\% of the RF area. The $10^{-14}$
s timing collapse therefore becomes a 5\% flip-angle error, producing a
...
initially safe because the local block times are small (e.g. 1ms RF pulses).
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\linewidth]{imgs/komaMRI/fixed_pixel_grid_overlay.png}
\caption{Reconstructed images after both fixes. The spheres remain
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{imgs/komaMRI/fixed_t1_fit_vs_true.png}
\caption{Noise-free $T_1$ recovery after both fixes. Mean MAPE decreases from
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{imgs/komaMRI/fixed_recovery_curves_koma.png}
\caption{Recovery curves after both fixes. The simulated samples now lie on the
...
\centering
\small
\begin{tabularx}{\linewidth}{p{0.18\linewidth}p{0.25\linewidth}X X}
\toprule
...
\bottomrule
\end{tabularx}
\caption{Summary of KomaMRI timing-fix validation.}
