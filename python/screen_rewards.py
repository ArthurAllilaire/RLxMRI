...
# be ranked/plotted alongside freshly computed ones.
CSV_FIELDS = ["level", "config", "reward_mode", "time_penalty", "mape_alpha",
              "gamma", "policy", "mean_return", "final_mape", "mean_tr",
              "total_time", "n_blocks", "mape_spread", "action_repeat"]
...
class RewardCfg:
    ...
    @property
    def label(self) -> str:
        return f"{self.reward_mode}|λ={self.time_penalty:g}|α={self.mape_alpha:g}"
    ...
    for cfg in configs:
        ...
        for g in gammas:
            ...
            _train_eval_ppo(cfg, g, ppo_label, key, base_kwargs, seeds,
                            timesteps, results,
                            PPO, Monitor, DummyVecEnv, VecNormalize)
