...
```
/
├── src/                        Julia package: QalibreMDPhantom
│   ├── QalibreMDPhantom.jl     Package root — exports everything
...
│   ├── geometry/
│   │   ├── sphere.jl           Voxelised sphere → Phantom
│   │   └── plate_layouts.jl    T1/T2/PD plate geometry
│   ├── materials/
│   │   ├── t1_array.jl         NiCl₂ T1 values at 1.5T and 3T
│   │   ├── t2_array.jl         MnCl₂ T2 values
│   │   ├── pd_array.jl         PD sphere values
│   │   ├── background.jl       Background water
│   │   └── fiducial.jl         Fiducial marker values (PLACEHOLDER — not real yet)
│   ├── sequences/
│   │   └── blocks.jl           ir_sequence(), se_sequence(), generalized_ir_signal(),
│   │                           single_spin_phantom(), rf_duration()
│   ├── fitting/
│   │   └── fits.jl             fit_t1_generalized_ir() — Levenberg–Marquardt T1 fitter
│   ├── baselines/
│   │   └── e0.jl               E0 conventional baseline sequences
│   └── rl/
│       └── e1.jl               E1Env struct + e1_reset!/e1_step! + juliacall aliases
│
├── python/
│   ├── qalibremd_gym/
│   │   ├── env.py              QalibreMDE1Env — Gymnasium wrapper over juliacall
│   │   └── juliapkg.json       Julia dep spec for the Python side
│   ├── train_e1.py             PPO training script (Stable-Baselines3)
│   ├── eval_e1.py              Evaluation + metrics
│   └── baseline_e1.py         Fixed-grid baseline comparison
│
├── runs/
│   └── e1/ppo/                 Trained E1 policy + eval history + tensorboard logs
│
├── examples/
│   ├── e0_baseline.jl          E0 demo script
│   └── plot_phantom.jl         3D phantom visualisation
│
├── test/                       Julia test suite (~373 tests, all green)
│   └── runtests.jl
│
├── docs/                       Background reading + experiment notes
├── project_context/            Supervisor emails, meeting notes, marking docs, timeline
├── meeting_notes/              Raw + polished meeting notes (M1.md, M1_polished.md)
│
├── PLAN.md                     Master experiment plan
├── E2_PLAN.md                  Current week's execution plan
└── Project.toml / Manifest.toml  Julia package manifest
```
...
- `QalibreMDPhantom` — this repo's package; builds the digital twin
...
- Julia 1.11 for the Python-facing runtime (juliacall 0.9.x constraint); Julia 1.12 for direct Julia use
...
**Important env vars for Python scripts:**
```bash
...
|---|---|---|---|
| E0 | Conventional IR-TSE + multi-TE SE baseline | **Done** | `src/baselines/e0.jl` |
...
- **Fiducial calibration values** in `src/materials/fiducial.jl` are placeholders. Not blocking for E1–E3, but must be replaced before any sim-to-real comparison.
